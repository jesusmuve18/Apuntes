...
int main()
{
	...
	double km_h, min_km;
	bool datos_ok;
	bool h_inic_ok, m_inic_ok, s_inic_ok;
	bool distancia_ok;
	...
	h_inic_ok=(0<=h_inicio && h_inicio<MAX_H);
	m_inic_ok=(0<=m_inicio && m_inicio<MAX_M),
	s_inic_ok=(0<=s_inicio && s_inicio<MAX_S);
	distancia_ok=(distancia>0);
	datos_ok=(h_inic_ok&&m_inic_ok&&s_inic_ok&&distancia_ok);
	...
	if(datos_ok)
	{
		...
		while(dorsal>=0)
		{
			...
			s_duracion_aux= s_final_aux-s_inicio_tot; //Calculo la duracion en 
													  //seg
			h_aux=s_duracion_aux/SEGS_H; //Paso los seg a horas, mins y segs
	...
	{
		if(!h_inic_ok)
			cout<<"Hora inicial no valida"<<endl;
		if(!m_inic_ok)
			cout<<"Minuto inicial no valido"<<endl;
		if(!s_inic_ok)
			cout<<"Segundo inicial no valido"<<endl;
		if(!distancia_ok)
			cout<<"Distancia no valida"<<endl;
	}
