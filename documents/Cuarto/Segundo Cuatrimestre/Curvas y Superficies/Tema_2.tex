...
\begin{propiedades}\
    \begin{enumerate}
        ...
        \begin{gather*}
            ...
            \right.
        \end{gather*}
        A estas expresiones las llamaremos \textbf{ecuaciones de Frenet}.
    \end{enumerate}
\end{propiedades}
...
\begin{ejemplo}\
    \begin{enumerate}
        ...
        \Func{\beta}{\bb{R}}{\bb{R}^2}{s}{(a_1 + r\cos(\frac{s}{r}), a_2 + r\sen (\frac{s}{r}))}
        Al derivar esta curva obtenemos 
        \begin{gather*}
            \beta'(s) = \left(-\sen\left(\frac{s}{r}\right), \cos\left(\frac{s}{r}\right)\right)
        \end{gather*}
        Estudiamos ahora el vector tangente y normal obteniendo
        \begin{gather*}
            \left\{
                \begin{array}{c}
                    T_\beta(s) = \left(-\sen\left(\dfrac{s}{r}\right), \cos\left(\dfrac{s}{r}\right)\right)\\\\
                    N_\beta(s) =  \left(-\cos\left(\dfrac{s}{r}\right), -\sen\left(\dfrac{s}{r}\right)\right)
                \end{array}
            \right.
        \end{gather*}
        Al derivar el vector tangente obtenemos 
        \begin{gather*}
            T_\beta'(s) = \frac{1}{r}\left(-\cos\left(\frac{s}{r}\right), -\sen\left(\frac{s}{r}\right)\right)
        \end{gather*}
...
\begin{ejercicio}
    Supongamos que $\alpha:I\to \bb{R}^2$ es una curva plana regular. Tomamos una reparametrización de $\alpha$ por el arco. Fijamos $a\in I$ y tenemos
    \begin{gather*}
        \xymatrix{
            J  \ar[r]^{S_a^{-1}} & I \ar[r]^\alpha &\bb{R}^2 
        }
    \end{gather*}
    Podemos considerar entonces $\beta = \alpha \circ S_a^{-1}$ una p.p.a. (creciente). Definimos la curvatura de $\alpha$ en un instante $t\in I$ como 
    \begin{gather*}
    ...
    De la definición de $\beta$ tenemos que $\beta \circ S_a(t) = \alpha(t)$ para todo $t\in I$. Derivando con la regla de la cadena obtenemos
    \begin{gather*}
        S_a'(t)T_\beta(S_a(t)) = \alpha'(t)
    \end{gather*}
    Aplicando $J$ el giro de $90^\circ$ en sentido antiohorario tenemos
    \begin{gather*}
        S_a'(t)N_\beta(S_a(t)) = J\alpha'(t)
    \end{gather*}
    Derivamos la expresión previa a aplicar $J$ aplicando la regla de Leibnitz y la regla de la cadena
    \begin{gather*}
        S_a''(t)T_\beta(S_a(t)) + S_a'(t)^2k_\beta(S_a(t))N_\beta(S_a(t)) = \alpha''(t)
    \end{gather*}
    ...
    \begin{gather*}
        \langle J\alpha'(t), \alpha''(t)\rangle = \langle (-a_2, a_1), (b_1, b_2) \rangle = -a_2b_1 + a_1b_2 = 
        \begin{vmatrix}
            a_1 & a_2 \\ b_1 & b_2
        \end{vmatrix} = det(\alpha'(t), \alpha''(t))
    \end{gather*}
    Por lo que podemos despejar la expresión anterior concluyendo que 
    \begin{gather*}
        k_\alpha(t) = \frac{det(\alpha'(t), \alpha''(t))}{|\alpha'(t)|^3}
    \end{gather*}
    Nos preguntamos ahora qué hubiese ocurrido si hubiésemos tomado una reparametrización decreciente. Para ello consideramos $\alpha:(-a,a) \to \bb{R}$ regular y $\beta:(-a,a) \to \bb{R}$ donde $\beta(t)=\alpha(-t)$. En este caso tendremos
    \begin{gather*}
        \beta'(t) = -\alpha'(-t)\\
        \beta''(t) = \alpha''(-t)
    \end{gather*}
...
\begin{ejercicio}
    Calcular la curvatura de la espiral logarítmica.
\end{ejercicio}

\begin{ejercicio}
    Estudiar longitud y curvatura para las gráficas de funciones derivables $f: I \to \bb{R}$.\\

    Hay una curva parametrizada correspondiente a $f$, $\alpha_f$ tal que $tr(\alpha_f) = Gr(f)$. Recordamos que $\alpha_f$ está definida como
    \Func{\alpha_f}{I}{\bb{R}^2}{t}{\alpha_f(t)=(t,f(t))}
    ...
    \begin{gather*}
        k_{\alpha_f}(t) = \frac{\begin{vmatrix}
            1 & f'(t) \\ 0 & f''(t)
        \end{vmatrix}}{\sqrt{1+f'(t)^2}^3} = \frac{f''(t)}{\sqrt{1+f'(t)^2}^3}
    \end{gather*}
\end{ejercicio}

\subsection{Invarianza de la curvatura por movimientos rígidos}

Nos preguntamos ahora qué ocurre con la curvatura y los movimientos rígidos. Recordamos que un movimiento rígido es una función $M:\bb{R}^2\to \bb{R}^2$ que viene dado por 
\begin{gather*}
    Mx = Ax + b
\end{gather*}
donde $A\in O(2)$, $b\in \bb{R}^2$. Definimos la deformación $\beta = M\circ \alpha$ y calculamos
\begin{gather*}
    k_\beta(t) = \frac{det(\beta'(t), \beta''(t))}{|\beta'(t)|^3}
\end{gather*}
Calculamos cada derivada por separado:
\begin{align*}
    \beta(t) &= M(\alpha(t)) = A\alpha(t)+b\\
    \beta'(t) &= A\alpha'(t)\\
    \beta''(t) &= A\alpha''(t)
\end{align*}
...
\begin{gather*}
    k_\beta(t) = \left\{
        \begin{array}{r c l}
            ...
            -k_\alpha(t) & \text{ si } & M \text{ es inverso}\\
        \end{array}
    \right.
\end{gather*}

\subsection{Teorema Fundamental de la Teoría Local de Curvas Planas}
\begin{teo}[Teorema Fundamental de la Teoría Local de Curvas Planas]
    Sea $I\subset \bb{R}$ un intervalo abierto y $k_0:\to \bb{R}$ una función diferenciable ($k_0\in C^\infty(I)$). Entonces existe una curva p.p.a. $\alpha:I \to \bb{R}^2$ tal que 
    \begin{gather*}
        k_\alpha(s) = k_0(s)\ \ \ \ \forall s\in I
    \end{gather*}
    Además, $\alpha$ es única salvo movimiento rígido directo.
    \begin{proof}
        En primer lugar probaremos la existencia de forma constructiva. Para ello definimos $\theta : I \to \bb{R}$ dada por 
        \begin{gather*}
            \theta(s) = \int_{s_0}^{s} k_0(t) dt \ \ \ \ \forall s \in I
        \end{gather*}
        para un $s_0\in I$ arbitrario. Por su definición tenemos que $\theta$ es diferenciable. Definimos de esta forma $\alpha : I \to \bb{R}^2$ como 
        \begin{gather*}
            \alpha(s) = \left( \int_{s_1}^{s} \cos(\theta(t)) dt, \int_{s_2}^s \sen(\theta(t)) \right)
        \end{gather*}
        con $s_1,s_2\in I$ arbitrarios. Según su definición tenemos que $\alpha$ es una curva. Además, tenemos que 
        \begin{gather*}
            \alpha'(s) = (\cos(\theta(s)), \sen(\theta(s))) \Rightarrow
            |\alpha'(t)| = 1 \ \ \ \ \forall s \in I
        \end{gather*}
        por lo que $\alpha$ es una curva p.p.a. Calculamos ahora el vector normal y el tangente:
        \begin{gather*}
            N_\alpha(s) = JT_\alpha(s) = (-\sen(\theta(s)), \cos(\theta(s)))\\
            T_{\alpha'}(s) = \theta(s)(-\sen(\theta(s), \cos(\theta(s))))
        \end{gather*}
        Por lo que la curvatura nos queda
        \begin{gather*}
            k(s) = \theta'(s) = k_0(s)\ \ \ \forall s \in I
        \end{gather*}
        por lo que ya tendríamos probada su existencia.\\

        % Se nos anima a hacer un programa al que, dándole la curvatura y los grados de libertad (pueden ser iguales) sea capaz de dibujar la curva.

        Veamos ahora la unicidad. Supongamos que tenemos dos curvas $\alpha, \beta : I \to \bb{R}^2$ p.p.a. tales que 
        \begin{gather*}
            k_\alpha(s) = k_\beta(s) = k_0(s)\ \ \ \ \forall s \in I
        \end{gather*}
        Sea $s_0\in I$, $\alpha(s_0),\beta(s_0)\in \bb{R}^2$, $b=\alpha(s_0)-\beta(s_0)$. Sean 
        \begin{gather*}
            \{T_{\alpha(s_0)}, N_{\alpha(s_0)}\}\\
            \{T_{\beta(s_0)}, N_{\beta(s_0)}\}
        \end{gather*}
        los diedros de Frenet correspondientes a $\alpha$ y $\beta$ respectivamente. Sea $A\in O(2)$ con $det(A)=1$ tal que 
        \begin{gather*}
            AT_{\beta(s_0)} = T_{\alpha(s_0)}\\
            AN_{\beta(s_0)} = N_{\alpha(s_0)}
        \end{gather*}
        Defino $M:\bb{R}^2\to \bb{R}^2$ por $Mx=Ax+b$. Denotamos $\gamma = M\beta$ (nos olvidamos de $\beta$ porque difiere de $\gamma$ en un movimiento rígido directo). Ahora comparamos $\alpha$ y $\gamma$. Tenemos que 
        \begin{gather*}
            k_\gamma(s) = k_\beta(s) = k_\alpha(s) = k_0(s)\\
            \gamma(s_0) = A\beta(s_0) + b = \alpha(s_0)
        \end{gather*}
        Debíamos haber elegido primero $A$ para que el tangente en $\beta$ vaya al tangente en $\alpha$ y para que el normal en $\beta$ vaya al normal en $\alpha$. Una vez hecho esto defino $b=\alpha(s_0) - A\beta(s_0)$ para que cuadren los cálculos. Además, 
        \begin{gather*}
            T_\gamma(s_0) = AT_\beta(s_0) = T_\alpha(s_0)\\
            N_\gamma(s_0) = N_\alpha(s_0)
        \end{gather*}
        Defino $f:I\to \bb{R}$ dado por 
        \begin{gather*}
        ...
        \begin{align*}
            ...
            &= k_0(s) \langle N_\alpha(s) - N_\gamma(s), T_\alpha(s)-T_\gamma(s) \rangle - k_0(s) \langle N_\alpha(s) - N_\gamma(s), T_\alpha(s)-T_\gamma(s) \rangle =\\
            &= 0
        \end{align*}
        Por lo que $f(s)=c\in \bb{R}$. Por la condición:
        \begin{gather*}
            f(s_0) = 0
        \end{gather*}
        Tenemos que $c=0$ por lo que 
        \begin{gather*}
            T_\alpha(s) = T_\gamma(s)\ \ \ \ \forall s \in I
        \end{gather*}
        Lo que nos dice que 
        \begin{gather*}
            \alpha'(s) = \gamma'(s)
        \end{gather*}
        Lo que además nos dice que $\alpha'(s) = \gamma'(s) + d$ con $d\in \bb{R}^2$ para todo $d\in \bb{R}^2$. De nuevo, como se tiene que verificar
        \begin{gather*}
            \alpha(s_0) = \gamma(s_0)
        \end{gather*}
        Tenemos que $d=0$ lo que finalmente nos dice que $\alpha = \gamma$, o equivalentemente, $\alpha$ difiere de $\beta$ en un movimiento rígido directo.        
    \end{proof} 
\end{teo}
