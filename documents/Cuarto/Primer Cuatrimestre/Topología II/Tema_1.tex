...
\begin{lema}
    ...
    \begin{proof}
        Podemos entender de forma gráfica la demostración de la siguiente forma
...
\begin{ejemplo}\
    \begin{enumerate}
        ...
        \end{gather*}
        y además $U_1$ es homeomorfo a $\bb{S}^2$ por lo que $\pi_1(U)$ es el trivial. De forma análoga si consideramos $U=X\setminus \{(0,0,-1)\}$ se tiene que $\pi_1(V)$ es trivial. Claramente se tiene que $U\cap V$ es arcoconexo\footnote{en un examen habrá que demostrarloo de forma más rigurosa} y por el teorema anterior se tiene que $X$ es simplemente conexo, luego su grupo fundamental es el trivial.
    \end{enumerate}
\end{ejemplo}
