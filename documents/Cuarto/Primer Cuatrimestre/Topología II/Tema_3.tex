...
\section{Superficies topológicas}
...
\begin{definicion}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[scale=1.5]
            ...
            \filldraw[draw=red, fill=red!20, thick] (0,0) -- (1,0) -- (0,1) -- cycle;
        \end{tikzpicture}
    \end{figure}
...
\begin{definicion}
    A cada presentación poligonal $\cc{P}$ le asociaremos un espacio topológico que denotaremos por $|\cc{P}|$ y que llamaremos la \textbf{realización geométrica de la presentación poligonal}. Su construcción es la siguiente
    \begin{enumerate}
...
\subsubsection{El grupo fundamental del $n$-toro}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \begin{scope}[shift={(7,0)}]
            ...
            \filldraw[fill=red, draw=red] (0,0) circle (1.5pt) node[below] {$x_0$};
        \end{scope}
    \end{tikzpicture}
\end{figure}
...
\begin{gather*}
    \pi_1(\bb{T}_n) = \frac{F(a_1,b_1,...,a_n,b_n)}{\langle i_{U*}([\alpha])\rangle_N} = \frac{F(a_1,b_1,...,a_n,b_n)}{\langle ab_1a_1^{-1}b^{-1}...a_nb_na_n^{-1}b_n^{-1} \rangle_N}
\end{gather*}
Este grupo no será conmutativo en general pero sí lo será para el 1-toro, en cuyo caso tendremos que 
\begin{gather*}
    \pi_1(\bb{T}_n) = \frac{F(a_1,b_1)}{\langle a_1b_1a_1^{-1}b^{-1}\rangle_N}
\end{gather*}
y en el cociente se verificará que $[a_1][b_1][a_1]^{-1}[b_1]^{-1} = 1$ por lo que se tendrá que $[a_1][b_1] = [b_1][a_1]$.\\

\subsubsection{El grupo fundamental de $\bb{R}\bb{P}_n^2$}
Pasamos ahora a calcular el grupo fundamental de $\bb{R}\bb{P}_n^2$ de forma análoga a como lo hicimos para el $n$-toro.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        % Lado 1
        \draw[thick] (A1) -- (A2) coordinate[midway] (M1);
        \node[above] at (M1) {$a_1$};
        \pgfmathsetmacro{\ang}{90 + 180/\n + 0*360/\n}
        \draw[->, thick, rotate around={\ang:(M1)}] (M1) ++(0, -0.01) -- ++(0,0.01);

        % Lado 2
        \draw[thick] (A2) -- (A3) coordinate[midway] (M2);
        \node[left] at (M2) {$a_1$};
        \pgfmathsetmacro{\ang}{90 + 180/\n + 1*360/\n}
        \draw[->, thick, rotate around={\ang:(M2)}] (M2) ++(0, -0.01) -- ++(0,0.01);

        % Lado 3
        \draw[thick] (A3) -- (A4) coordinate[midway] (M3);
        \node[below left] at (M3) {$a_2$};
        \pgfmathsetmacro{\ang}{90 + 180/\n + 2*360/\n}
        \draw[->, thick, rotate around={\ang:(M3)}] (M3) ++(0, -0.01) -- ++(0,0.01);

        % Lado 4
        \draw[thick] (A4) -- (A5) coordinate[midway] (M4);
        \node[below] at (M4) {$a_2$};
        \pgfmathsetmacro{\ang}{90 + 180/\n + 3*360/\n}
        \draw[->, thick, rotate around={\ang:(M4)}] (M4) ++(0, -0.01) -- ++(0,0.01);

        % Lado 5
        \draw[thick, dashed] (A5) -- (A6) coordinate[midway] (M5);
        % \node[below right] at (M5) {$a_2$};
        % \pgfmathsetmacro{\ang}{90 + 180/\n + 4*360/\n}
        % \draw[->, thick, rotate around={\ang:(M5)}] (M5) ++(0, -0.01) -- ++(0,0.01);

        % Lado 6
        \draw[thick] (A6) -- (A7) coordinate[midway] (M6);
        \node[right] at (M6) {$a_n$};
        \pgfmathsetmacro{\ang}{90 + 180/\n + 5*360/\n}
        \draw[->, thick, rotate around={\ang:(M6)}] (M6) ++(0, -0.01) -- ++(0,0.01); 

        % Lado 7
        \draw[thick] (A7) -- (A1)coordinate[midway] (M7);
        \node[above] at (M7) {$a_n$};
        \pgfmathsetmacro{\ang}{90 + 180/\n + 6*360/\n}
        \draw[->, thick, rotate around={\ang:(M7)}] (M7) ++(0, -0.01) -- ++(0,0.01);

        \node at (0,0) {$P_0$};
    \end{tikzpicture}
\end{figure}

$\bb{R}\bb{P}_n^2$ será $P_0$ bajo la relación de equivalencia $R$ que identifica a los lados con igual nombre, es decir
\begin{gather*}
    \bb{R}\bb{P}_n^2 = \frac{P_0}{R}
\end{gather*}
Buscamos aplicar Seifert-van Kampen y tomamos 
\begin{gather*}
    U=\bb{R}\bb{P}_n^2 \setminus \{x_0\}\\
    V \equiv \text{ disco abierto centrado en $x_0$ que no toque las aristas}
\end{gather*}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \begin{scope}
            ...
            \fill[red!20] (A1) -- (A2) -- (A3) -- (A4) -- (A5) -- (A6) -- (A7) -- cycle;
...
Es claro que $\pi_1(V)$ es trivial por ser un disco. Para calcular el grupo fundamental de $U$ comenzamos considerando un retracto de deformación suyo. Es fácil ver que el borde (en el cociente) es retracto de deformación de $U$. Además, todos los vértices del borde tendrán la misma clase de equivalencia bajo $R$. Para ver esto último vamos a numerar los vértices como $v_{ij}$ con $i\in \{1,2\}$ y $j\in \{1,...,n\}$ de forma que $v_{1j}$ sea el principio de la primera vez que aparece $a_j$ y $v_{2j}$ el principio de la segunda vez que aparece la arista $a_j$ para todo $j\in \{1,...,n\}$. Es decir,
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \filldraw (-4,0) circle (1.5pt) node[above] {$v_{1j}$};
        \filldraw (-2,0) circle (1.5pt) node[above] {$v_{2j}$};
        \filldraw (0,0) circle (1.5pt) node[above] {$v_{1,j+1}$};
        ...
        % Lados
        \node[label=below:{$a_j$}] at (-3,0) {};
        \node[label=below:{$a_j$}] at (-1,0) {};
    \end{tikzpicture}
\end{figure}

Entonces tenemos que 
\begin{gather*}
    [v_{1j}] = [v_{2j}] \text{ por ser principio de la 1ª $a_j$  y el principio de la 2ª $a_j$ respectivamente}\\
    [v_{2j}] = [v_{1,j+1}] \text{ por ser el final de la 1ª $a_j$ y el final de la 2ª $a_j$ respectivamente}\\
\end{gather*}
Como la $j$ es arbitraria se verificará para todo $j\in\{1,...,n\}$ (entendiendo que $n+1$ vuelve a ser el $1$) y por tanto todos los vértices son de la misma clase de equivalencia. Nos quedará algo como 
...
Aplicando Seifert-van Kampen tenemos que 
\begin{gather*}
    \pi_1(\bb{R}\bb{P}_n^2) = \frac{F(a_1,...,a_n)}{\langle i_{U*}([\alpha]) \rangle_N} = \frac{F(a_1,...,a_n)}{\langle a_1a_1...a_na_n \rangle_N}
\end{gather*}

\begin{definicion}
    Recordemos que dado un grupo $G$ se define el \textbf{conmutador} de $G$ por 
    \begin{gather*}
        [G:G] = \langle \{ghg^{-1}h^{-1} : g,h\in G\}\rangle_N
    \end{gather*}
    El \textbf{grupo abelianizado} de $G$ es 
    \begin{gather*}
        \frac{G}{[G:G]}
    \end{gather*}
\end{definicion}
...
\begin{definicion}
    En el caso de un espacio topológico $X$, el abelianizado de $\pi_1(X)$ (suponiendo que es arcoconexo) se suele llamar \textbf{primer grupo de homología} de $X$ y se denota por $H_1(X)$ (o bien $H_1(X, \bb{Z})$).
\end{definicion}

\begin{observacion}
    Dos grupos $G_1$ y $G_2$ podrían tener el mismo grupo abelianizado aunque no sean isomorfos.
\end{observacion}

\begin{coro}
    El primer grupo de homología de las superficies compactas es 
    \begin{gather*}
        H_1(\bb{S}^2) = \{0\}\\
        H_1(\bb{R}\bb{P}^2) = \bb{Z}_2\\
        H_1(\bb{T}_n) = \bb{Z} \times \overset{(2n)}{...} \times \bb{Z}\\
        H_1(\bb{R}\bb{P}_n^2) = \bb{Z} \times \overset{(n-1)}{...} \times \bb{Z} \times \bb{Z}_2
    \end{gather*}
\end{coro}

\begin{coro}
    Si $S$ es una superficie conexa y compacta, entonces $S$ es homeomorfa a una y solo una de las siguientes superficies
    \begin{gather*}
        \bb{S}^2,\ \bb{T}_n \ \text{ o } \bb{R}\bb{P}_n^2,\ \text{ con }n\geq 1
    \end{gather*}
\end{coro}

\section{Característica de Euler y orientabilidad}

\begin{definicion}
    Dada una presentación poligonal $\cc{P}$ denotamos por $V$, $A$ y $C$ al número de vértices, de aristas y caras (o número de polígonos de $\cc{P}$) respectivamente donde el número de vértices y aristas está visto en el espacio cociente $|\cc{P}|$. Definimos entonces la \textbf{característica de Euler} de $\cc{P}$ como 
    \begin{gather*}
        \chi(\cc{P}) = V-A+C
    \end{gather*} 
\end{definicion}
...
\begin{ejemplo}\
    \begin{enumerate}
        \item Consideramos la presentación poligonal de $\bb{S}^2$, $\cc{P}=\{a,b;\ aa^{-1}bb^{-1}\}$:
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                ...
                % Vértices 
                \filldraw(0,0) circle (1.5pt) node[below left] {$v_1$};
                \filldraw(1.5,0) circle (1.5pt) node[below right] {$v_2$};
                \filldraw(1.5,1.5) circle (1.5pt) node[above right] {$v_3$};
                \filldraw(0,1.5) circle (1.5pt) node[above left] {$v_4$};
            \end{tikzpicture}
        \end{figure}
        ...
        \begin{gather*}
            ...
            V=3
        \end{gather*}
        Para contar el número de vértices hemos tenido en cuenta que $[v_2]=[v_4]$ por ser el principio de $a^{-1}$ y $a$ respectivamente. Por tanto tenemos que 
        \begin{gather*}
            \chi(\cc{P}) = 1-2+3 = 2
        \end{gather*}

        \item Consideramos ahora la presentación $\cc{P}=\{a,b,c,d;\ abcd,\ abd^{-1}c^{-1}\}$
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                ...
                \begin{scope}[shift={(4,0)}]
                    ...
                    \node[label=left:{$a$}] at (0, 0.75) {}; %1
                    \node[label=below:{$b$}] at (0.75, 0) {}; %2
                    \node[label=right:{$d^{-1}$}] at (1.5, 0.75) {}; %3
                    \node[label=above:{$c^{-1}$}] at (0.75, 1.5) {}; %4
                    ...
                    % Vértices 
                    \filldraw(0,0) circle (1.5pt) node[below left] {$v_5$};
                    \filldraw(1.5,0) circle (1.5pt) node[below right] {$v_6$};
                    \filldraw(1.5,1.5) circle (1.5pt) node[above right] {$v_7$};
                    \filldraw(0,1.5) circle (1.5pt) node[above left] {$v_8$};
                \end{scope}
            \end{tikzpicture}
        \end{figure}
        Tendremos ahora 
        \begin{gather*}
            ...
            V=3
        \end{gather*}
        Donde para contar el número de vértices hemos tenido en cuenta que
        \begin{gather*}
            [v_4] = [v_5] = [v_2] = [v_8]
        \end{gather*}
        Por tanto 
        ...
        \item Consideramos ahora la presentación poligonal de $\bb{R}\bb{P}^2$, $\cc{P}=\{a,b; aa^{-1}bb\}$
        ...
        \begin{gather*}
            ...
            V=2
        \end{gather*}
        Donde hemos tenido en cuenta que $[v_2]=[v_4]=[v_3]$.
        Por tanto 
        \begin{gather*}
            \chi(\cc{P}) = 1-2+2 = 1
        \end{gather*}

        \item Pensamos ahora en la presentación del $n$-toro,
        \begin{gather*}
            \cc{P} = \{a_1,b_1,...,a_n,b_n;\ a_1b_1a_1^{-1}b_1^{-1}...a_nb_na_n^{-1}b_n^{-1}\}
        \end{gather*}
        ...
        Donde ya demostramos anteriormente que solo hay un vértice por lo que tendremos 
        \begin{gather*}
            C=1\\
            A=2n\\
            V=1
        ...
        \begin{gather*}
            \chi(\cc{P}) = 1-2n+1 = 2-2n = 2(1-n)
        \end{gather*}

        \item Consideramos la presentación de $\bb{R}\bb{P}_n^2$, $\cc{P}=\{a_1,...,a_n;\ a_1a_1...a_na_n\}$.
        ...
        Donde tenemos ya probado que solo hay un vértice por lo que 
        \begin{gather*}
            C=1\\
            A=n\\
            V=1
        \end{gather*}
        Lo que nos dice que $\chi(\cc{P}) = 1-n+1 = 2-n$
    \end{enumerate}
\end{ejemplo}
...
\begin{coro}
    Dada una superficie compacta, cualquiera la cararacterística de Euler de cualquier presentación suya es la misma.
\end{coro}

\begin{definicion}
    Dada una superficie compacta $S$ definimos su \textbf{característica de Euler} como la característica de Euler de cualquier presentación suya y la denotaremos por $\chi(S)$.
\end{definicion}

\begin{observacion}
    Con los ejemplos anteriores podemos llegar a que 
    \begin{gather*}
        \chi(\bb{S}^2) = 2\\
        \chi(\bb{R}\bb{P}^2) = 1\\
        \chi(\bb{T}_n) = 2-2n\ (\leq 0),\ n\geq 1\\
        \chi(\bb{R}\bb{P}_n^2) = 2-n\ (\leq 0),\ n\geq 2
    \end{gather*}
    Si la superficie que queremos estudiar tuviera característica positiva o impar sería muy fácil de clasificar. Si por el contrario tuviéramos característica $0$ o un número negativo par aún no sabríamos clasificarlo.
\end{observacion}

\begin{definicion}
    Decimos que una presentación poligonal $\cc{P}$ es \textbf{no orientada} si en alguna de sus expresiones aparece 2 veces un símbolo con el mismo exponente, es decir, existirá un $w_i$ de la forma
    \begin{gather*}
        w_i=...a...a...
    \end{gather*}
    Decimos que una superficie compacta es \textbf{no orientable} si tiene una presentación poligonal no orientada.
\end{definicion}

\begin{prop}
    Sea $S$ una superficie compacta y conexa, entonces son equivalentes:
    \begin{enumerate}
        \item $S$ es orientable.
        \item $S$ tiene una presentación poligonal con una sola expresión que es orientada.
        \item Cualquier presentación poligonal de $S$ de una única expresión es orientada.
    \end{enumerate}
\end{prop}

Nos planteamos ahora estudiar cuáles son las superficies compactas orientables y con las presentaciones que conocemos tenemos el siguiente resultado:
\begin{gather*}
    \begin{array}{r c  l}
        aa^{-1}bb^{-1} \text{ es una presentación de } \bb{S}^2 & \Rightarrow & \bb{S}^2 \text{ es orientable}\\
    aabb \text{ es una presentación de } \bb{R}\bb{P}^2 & \Rightarrow & \bb{R}\bb{P}^2 \text{ es no orientable}\\
    a_1b_1a_1^{-1}b_1^{-1}...a_nb_na_n^{-1}b_n^{-1} \text{ es una presentación de } \bb{T}_n & \Rightarrow & \bb{T}_n \text{ es orientable ($n\geq 1$)}\\
    a_1a_1...a_na_n \text{ es una presentación de } \bb{R}\bb{P}^n  & \Rightarrow & \bb{R}\bb{P}^n \text{ es no orientable ($n\geq 2$)}
    \end{array}
\end{gather*}
Por tanto tenemos ya una manera de distinguir entre las superficies con característica de Euler $0$ o negativa par estudiando la orientabilidad de la superficie.\\

\begin{coro}
    La caracteristica de Euler y la orientabilidad de una superficie compacta $S$ la determinan. 
\end{coro}
