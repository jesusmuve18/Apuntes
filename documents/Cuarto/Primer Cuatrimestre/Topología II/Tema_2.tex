...
\begin{ejemplo}\
    \begin{enumerate}
        ...
        \begin{gather*}
            ...
            = (\cos(a\theta + c\varphi), \sen(a\theta + c\varphi), \cos(b\theta + d\varphi), \cos(b\theta + d\varphi))
        \end{gather*}
...
\begin{coro}
    Sean $(R_1, p_1)$, $(R_2, p_2)$ dos recubridores de un espacio topológico $B$, $b_0\in B$, $r_1\in p_1^{-1}(b_0)$ u $r_2\in p_2^{-1}(b_0)$. Si se verifica que
    \begin{gather*}
        p_{1*}(\pi_1(R_1, r_1)) \subseteq p_{2*}(\pi_1(R_2, r_2))
    \end{gather*}
...
\begin{ejemplo}\
    \begin{enumerate}
        ...
        \item Consideramos $S_n$ la circunferencia de $\bb{R}^2$ centrada en $\left(\frac{1}{n}, 0\right)$ y de radio $\frac{1}{n}$. Consideramos el siguiente expacio
        \begin{gather*}
            B = \bigcup\limits_{n\in \bb{N}} S_n
        \end{gather*}
        El espacio topológico $B$ no es semilocalmente simplemente conexo.
        ...
        Tenemos que $B$ es simplemente conexo por ser contráctil pero no es localmente simplemente conexo.
    \end{enumerate}
\end{ejemplo}
