...
\begin{observacion}
    ...
    \begin{enumerate}
        \item Supongamos que existes dos parametrizaciones p.p.a. $\alpha:I\to C$ y $\beta:J \to C$, que tenemos 
        \begin{gather*}
            W = \alpha(I) \cap \beta(J) \neq \emptyset, \qquad \alpha(I)\not\subset \beta(J),\qquad \beta(J)\not\subset \alpha(I)
        \end{gather*}
        ...
        \end{gather*}
        y por tanto existe una única traslación $T:\bb{R} \to \bb{R}$ tal que $\Phi_{|_{I_2}} = T_{|_{I_2}}$. Tendremos que 
        \begin{gather*}
            T(I_2) = J_1 \quad \Rightarrow \quad I_2 = T^{-1}(J_1)
        \end{gather*}
        Consideramos el conjunto $K=I\cup T^{-1}(J)$ y nos planteamos si $K$ es un intervalo, lo cual ocurrirá si $I\cap T^{-1}(J)\neq \emptyset$. Con el siguiente dibujo

        % Hacer dibujo

        Se puede ver que $I\cap T^{-1}(J) = I_2 = T^{-1}(J_1)$ por lo que $K$ es un intervalo abierto de $\bb{R}$. Definimos entonces $\gamma:K \to \bb{R}^2$ de la siguiente forma:
        \begin{gather*}
            \gamma(s) = \left\{
                \begin{array}{l c c}
        ...
        Sabemos que existe una única traslación $R:\bb{R}\to \bb{R}$ tal que 
        \begin{gather*}
            \Phi_{|_{I_1}} = R_{|_{I_1}} , \qquad R\neq T
        \end{gather*}
        y como se puede ver en el gráfico anterior %poner código gráfico
        se tiene
        \begin{gather*}
            R(I_1) = J_2 \quad \Rightarrow \quad I_1 = R^{-1}(J_2)
        \end{gather*}
        Consideramos así la composición $T^{-1} \circ R : \bb{R} \to \bb{R}$ que es claramente una traslación de $\bb{R}$. Vamos a tomar un punto cualquiera $s\in I_1$ y vamos a considerar 
        \begin{gather*}
        ...
        \end{gather*}
        es decir, que $\gamma$ actúa de forma periódica sobre $I_1$. Además, podemos extender $\gamma$ a una cierta $\delta : \bb{R} \to \bb{R}^2$ $a-$periódica con $a>0$. Además $a$ es el mínimo período (por unicidad de $T$ y $R$) de $\delta$ y $\delta_{|_{[0,a)}}$ es inyectiva.\\

        Todo esto nos dice que $tr(\delta)$ es una curva de Jordan y además
        \begin{gather*}
            tr(\delta) = tr(\gamma) = tr(\alpha) \cup tr(\beta) \subset C
        \end{gather*}
        y $tr(\alpha)$, $tr(\beta)$ son abiertos en $C$ por lo que su unión será también abierta por lo que $tr(\delta)$ será abierta y homeomorfa a $S^1$ que es compacto. Podemos concluir que $tr(\delta)$ es cerrado en $C$. Suponiendo que $C$ es conexa y sabiendo que la traza de una curva no puede ser vacía tenemos que $tr(\delta)=C$.\\

        Concluimos que $C$ ha de ser una curva de Jordan del Ejemplo 5.
    \end{enumerate}
