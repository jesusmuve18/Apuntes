...
@export var dibujar_normales : bool = true
...
func _init() -> void:
	...
	normales = Utilidades.calcNormales( vertices, triangulos )	
...
func _ready() -> void:	
	## Dibujar normales
	if(dibujar_normales):
		var normales_inst: MeshInstance3D = Utilidades2.dibujarNormalesMesh(vertices, normales, 0.5, Color(1.0, 1.0, 1.0, 1.0))
