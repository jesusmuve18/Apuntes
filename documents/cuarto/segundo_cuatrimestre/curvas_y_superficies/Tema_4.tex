...
\begin{ejercicio}\
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \end{gather*}
            por lo que $Ap \bot v$ para todo $v\in \langle p \rangle^\bot$. Para todo $p$ existirá un cierto $\lambda(p)$ tal que 
            \begin{gather*}
                Ap=\lambda(p)p
            \end{gather*}
            Además, $\lambda = cte = f(p) = \lambda(p)$ lo que significa que hemos podido diagonalizar la matriz (hemos obtenido los valores propios de $A$).

            \item Si $f(p_M)\neq f(p_m)$, entonces necesariamente $p_M\neq p_m$.
        \end{enumerate}
    \end{enumerate}
\end{ejercicio}