...
% La combinación de los cuatro parámetros genera cuatro regiones de movimiento posible para cada agente en cada dimensión, como ilustra la Figura~\ref{fig:geometry}. Dependiendo del signo de $\phi = r_1 \cdot f(r_2)$ y del valor de $r_3$, el nuevo punto puede caer en cualquiera de los cuatro cuadrantes definidos por los ejes que pasan por $X_i^t$ y $P_i^t$ (agente y destino respectivamente).
...
%     \draw[gray!50, thin] (-2.8,0) -- (7.2,0);
%     \draw[gray!50, thin] (0,-2.3) -- (0,2.3);

%     %% Distancia base |Delta|
%     \draw[<->, gray!70, thin] (0,-1) -- (5,-1)
%         node[midway, below, gray] {$|\Delta_i^t|$};
%     \draw[dashed, gray!70] (5,0) -- (5,-1.5);

%     %% --- Flecha R1: r1*f>0, en la dirección de P (dentro del segmento)
%     \draw[->, thick, myyellow] (0,0) -- (3.2,0)
%         node[above, myyellow] {\footnotesize $|\phi|\!<\!1$};

%     %% --- Flecha R2: r1*f>0, sobrepasa P (exploración)
%     \draw[->, thick, myblue] (0,0) -- (6.5, 1.25)
%         node[right, myblue] {\footnotesize $\phi\!>\!1$};

%     %% --- Flecha R3: r1*f<0 hacia atrás
%     \draw[->, thick, teal!70!black] (0,0) -- (-2.1, 0)
%         node[above, teal!70!black] {\footnotesize $\phi\!<\!-1$};

%     %% Zona explotación (segmento X a P)
%     \draw[myyellow!40, line width=6pt, opacity=0.5] (0,0) -- (5,0);
%     \node[myyellow, font=\footnotesize] at (2.5, -0.5)
%         {explotación};

%     %% Zona exploración (fuera del segmento)
%     \draw[mygreen!20, line width=6pt, opacity=0.4] (5,0) -- (6.8,0);
%     \node[mygreen, font=\footnotesize] at (6, -0.5)
%         {exploración};

%     %% Punto destino P
%   \filldraw[myred] (P) circle (3pt);
%   \node[above, myred] at (P) {$P_i^t$};

%   %% Punto actual X
%   \filldraw[myblue] (X) circle (3pt);
%   \node[above, myblue] at (X) {$X_i^t$};

% \end{tikzpicture}
% \caption{Posibles movimientos de un agente en una dimensión. La banda naranja representa el segmento $[X_i^t, P_i^t]$ (zona de explotación). Cuando $|\phi| < 1$, el agente aterriza dentro del segmento; cuando $|\phi| > 1$ (exploración), sobrepasa el destino. El signo negativo de $\phi$ permite moverse en sentido opuesto al destino, explorando el semiplano izquierdo.}
% \label{fig:geometry}
% \end{figure}

% Desde una perspectiva probabilística, en cada iteración el valor esperado de $|\phi| = |r_1 \cdot f(r_2)|$ decrece con $t$ al hacerlo $r_1$. Esto convierte la dinámica colectiva de la población en un proceso de {enfriamiento implícito}: en las primeras iteraciones los agentes realizan saltos amplios y divergentes; conforme avanza la búsqueda, los pasos se acortan y la población converge gradualmente hacia el punto destino, refinando la solución con precisión creciente.
...
\]
donde $|\Delta_j^t| \geq 0$ siempre. Por tanto, el signo y la magnitud del desplazamiento quedan determinados exclusivamente por $\phi$, que puede tomar cualquier valor real en $[-r_1,\, r_1]$. Esto da lugar a \textbf{cuatro regímenes de movimiento}, ilustrados en la Figura~\ref{fig:geometry}:
