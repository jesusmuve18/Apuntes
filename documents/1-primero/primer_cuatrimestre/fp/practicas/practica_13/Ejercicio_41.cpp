/****************************************************************************/
//	FUNDAMENTOS DE PROGRAMACIÓN			RELACIÓN_VI				EJERCICIO_41
/****************************************************************************/
/*
	...
	Reescriba la solución del ejercicio 32 de la Relación de Problemas IV 
	usando las clases Punto2D, Circunferencia y Rectangulo.
	...
	Entradas: Parámetros para crear un rectángulo

	Salidas: Todas las posibles circunferencias inscritas en el rectángulo
			 descrito empezando en un radio de 0.5 y aumentando 0.25 en cada
			 iteración.
...
int main()
{
	...
	const int MAX_CIRCUNFERENCIAS=100;
	
	Circunferencia coleccion[MAX_CIRCUNFERENCIAS]; //array de circunferencias
	
	LectorPunto2D lector; //inicializo el lector
	...
	//Genero una circunferencia con el radio por defecto centrada
	//en el cruce de diagonales del rectángulo
	Circunferencia circ(rect.GetCentro());
	
	
	int contador=0; //Cuenta el número de circunferencias inscritas
	
	//Voy comprobando si está inscrita en el rectángulo y aumentando  
	while(circ.GetDiametro()<=rect.GetBase() 
									&&circ.GetDiametro()<=rect.GetAltura())
	{
		coleccion[contador]=circ;
		
		circ.Aumenta(); //Aumento el valor por defecto
		contador++;
	}
	...
	//Muestro la cadena en caso de que haya alguna cadena guardada
	if(contador>0)
	{
		cout<<endl;
		cout<<"Circunferencias inscritas: "<<endl;
		cout<<endl;
		
		for(int i=0; i<contador; i++)
		{
			cout<<"    Circunferencia "<<i+1<<":"<<endl;
			cout<<coleccion[i].ToString()<<endl;
			cout<<endl;
		}
	}
	else
	{
		cout<<endl;
		cout<<"No hay ninguna circunferencia inscrita posible"<<endl;
		cout<<endl;
	}
