...
public:
	...
	/***********************************************************************/
	//Constructor a partir de un dato string
	
	Multiconjunto(string cadena, char separador=',')
	{

		int contador=0;
		cadena+=separador; //Me aseguro de que termine la lectura
		...
		for(int j=0; j<contador; j++)
		{
			...
			if(dominio.Contiene(numero))
			{
				multiplicidades.Modifica(dominio.Posicion(numero)
					,(multiplicidades.Elemento(dominio.Posicion
					(numero))+1));
				
				#ifdef DEBUG
				
				cout<<"multiplicidad de "<<numero<<" +1"<<endl;
				
				cout<<"Cambio "
					<<multiplicidades.Elemento(dominio.Posicion
					(numero))
					<<" de la posicion "<<dominio.Posicion(numero)
					<<" por "
					<<(multiplicidades.Elemento(dominio.Posicion
					(numero))+1)<<endl;
			...
			{
				dominio.Aniade(numero);
				multiplicidades.Aniade(1);
				
				#ifdef DEBUG
				
				cout<<"Aniado "<<numero<<" al dominio"
					<<endl;
				cout<<"Dominio: "<<dominio.ToString()<<endl;
	...
	/***********************************************************************/
	//Añade un elemento con una cierta multiplicidad
	
	void Aniade (int numero, int multiplicidad)
	{
		if(dominio.Contiene(numero))
		{
			multiplicidades.Modifica(dominio.Posicion(numero)
			...
			cout<<"multiplicidad de "<<numero<<" +"<<multiplicidad<<endl;
		...
		{
			dominio.Aniade(numero);
			multiplicidades.Aniade(multiplicidad);
	...
	/***********************************************************************/
	//Devuelve la secuencia correspondiente a la operación Soporte
	...
	/***********************************************************************/
	//Devuelve el numero de elementos distintos que hay
	
	int TotalUtilizados()
	{
		return dominio.TotalUtilizados();
	}
	
	
	/***********************************************************************/
	//Comprueba si un multiconjunto está contenido en otro
	
	bool Incluido(Multiconjunto conjunto)
	{
		bool incluido=(TotalUtilizados()<=conjunto.TotalUtilizados());
		
		int i=0;
		
		while(incluido && i<TotalUtilizados())
		{
			
			if(Multiplicidad(Soporte().Elemento(i))
				>conjunto.Multiplicidad(conjunto.Soporte().Elemento(i)))
			{
				incluido=false;
			}
	...
	/***********************************************************************/
	//Operación de intersección de 2 multiconjuntos
	
	Multiconjunto Interseccion(Multiconjunto conjunto)
	{
		Multiconjunto resultado;
		
		for(int i=0; i<TotalUtilizados(); i++)
		{
			if (conjunto.Soporte().Contiene(Soporte().Elemento(i)))
			{
				int min_multip;
				...
				if(Multiplicidad(Soporte().Elemento(i))
					<conjunto.Multiplicidad(conjunto.Soporte().Elemento(i)))
				{
					min_multip=Multiplicidad(Soporte().Elemento(i));
				}
				else
				{
					min_multip=conjunto.Multiplicidad(Soporte().Elemento(i));
				}
				
				
				resultado.Aniade(Soporte().Elemento(i),min_multip);
	...
	string ToString(string separador=" , ")
	{
		...
		for(int j=0; j<dominio.TotalUtilizados(); j++)
		{
			...
			for(int i=0; i<Multiplicidad(dominio.Elemento(j)); i++)
			{
				...
				cadena+=(to_string(dominio.Elemento(j)));
			}
...
int main()
{
	...
	Multiconjunto conjunto1("51,5,5,3,3,22");
	Multiconjunto conjunto2("51*5*5*5*3*3*22*6",'*');
	
	cout<<"Conjunto1: ("<<conjunto1.ToString()<<")"<<endl;
	cout<<"Soporte: ("<<conjunto1.Soporte().ToString()<<")"<<endl;
	
	cout<<endl;
	
	cout<<"Conjunto2: ("<<conjunto2.ToString()<<")"<<endl;
	cout<<"Soporte: ("<<conjunto2.Soporte().ToString()<<")"<<endl;
	
	cout<<endl;
	
	cout<<"Conjunto1 contenido en Conjunto2: "<<boolalpha
		<<conjunto1.Incluido(conjunto2)<<endl;
		
	cout<<"Conjunto2 contenido en Conjunto1: "<<boolalpha
		<<conjunto2.Incluido(conjunto1)<<endl;
	
	cout<<endl;
	cout<<"Interseccion: ("<<conjunto1.Interseccion(conjunto2).ToString()<<")"
		<<endl;
