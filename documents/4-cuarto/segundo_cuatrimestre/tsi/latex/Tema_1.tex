...
\begin{notacion}
    ...
    \begin{center}%
        \begin{tabular}{c | l}%
            $c(n_i, n_j)$ & \textbf{Coste óptimo} entre $n_i$ y $n_j$ \\
            $g(n)$ & \textbf{Costo actual} desde $s$ hasta $n$.\\
            $g^*(n)$ & \textbf{Costo óptimo} entre $s$ y $n$, es decir, $g^*(n) = c(s,n)$ para todo $n$.\\
            $g_P(n)$ & \textbf{Costo} desde $s$ hasta $n$ por el camino $P$.\\
            $h(n)$ & \textbf{Costo estimado} del nodo $n$ al objetivo $\gamma$.\\
            $h_P(n)$ & \textbf{Costo estimado} del nodo $n$ al objetivo $   \gamma$ por el camino $P$.\\
...
\begin{lema}\label{lema:admisible_2}
    Supongamos $h$ admisible y $A^*$ no ha terminado todavía. Entonces, para cualquier óptimo entre $s$ y $\Gamma$ tendremos que 
    \begin{gather*}
...
Por la desigualdad triangular tenemos que $c(s,\gamma) \leq c(s,n) + c(n, \gamma)$ para cualquier nodo $n$, cualquier nodo inicial $s$ y final $\gamma$. Tenemos por tanto que 
\begin{gather*}
    h^*(s) \leq c(s,n) + h^*(n) \Rightarrow h^*(n) \leq c(n,n') + h^*(n') \ \ \ \ \forall n,n' \ \text{ nodos}
\end{gather*}
\begin{definicion}
    Diremos que $h$ es \textbf{consistente} si se verifica que 
    \begin{gather*}
        h(n) \leq c(n,n') + h(n') \ \ \ \ \forall n,n' \ \text{ nodos}
    \end{gather*}
\end{definicion}
...
\begin{definicion}
    ...
    \begin{gather*}
        h(n)\leq c(n,n')+h(n') \ \ \ \ \forall n \text{ nodo },\ \ \forall n'\in suc(n)
    \end{gather*}
\end{definicion}
...
\begin{teo}
    ...
    \begin{proof}
        ...
        \begin{equation*}
            \text{ABIERTOS} = \{..., n, ...\}
        \end{equation*}
        Por el lema 1 tenemos que $\exists n'\in$ABIERTOS tal que $n'\in P^*_{s-n}$, en cuyo caso, $g(n')=g^*(n')$. Tenemos entonces
        \begin{gather*}
            f(n') = g(n') + h(n') \overset{\text{lema 1}}{=} g^*(n') + h(n') \overset{consist.}{\leq} g^*(n') + c(n',n) + h(n) \\
            \overset{opt.}{=} g^*(n)+h(n) \overset{hip.}{<} g(n)+h(n)=f(n)
        \end{gather*}
        y llegamos a contradicción ya que $A^*$ no habría seleccionado como mejor nodo $n$ teniendo un $n'\in ABIERTOS$ con $f(n')<f(n)$.
    \end{proof}
\end{teo}

\begin{observacion}
    Si la heurística es monótona no hay que revisar $CERRADOS$, alcanzamos eficiencia y el concepto de dominancia se basa en el tiempo.
\end{observacion}