(define (domain tierra-media)
  ...
  ; ACCIÓN: formarComunidad  (3 Hobbits + 1 Mago + 1 Elfo)
  ;
  ; Reúne a los 5 personajes que constituirán la Comunidad del Anillo
  ; (tres Hobbits, un Mago y un Elfo) en una misma localización.
  ;
  ; Parámetros:
  ;   ?p1  - Primer miembro  (Hobbit)
  ;   ?p2  - Segundo miembro  (Hobbit)
  ;   ?p3  - Tercer miembro  (Hobbit)
  ;   ?p4  - Cuarto miembro (Mago)
  ;   ?p5  - Quinto miembro (Elfo)
  ...
  ;   - ?p1, ?p2 y ?p3 son Hobbits, ?p4 es un Mago y ?p5 es un Elfo.
  ;   - ?p1 < ?p2 < ?p3 para romper simetrías.
  ;
  ; Efectos:
  ...
  (:action formarComunidad
    :parameters (?p1 - Personaje ?p2 - Personaje ?p3 - Personaje ?p4 - Personaje ?p5 - Personaje ?l - Localizacion)
    :precondition
      (and
        ...
        (disponible ?p4)           ; ?p4 está libre
        (disponible ?p5)           ; ?p5 está libre
        ...
        (esDeTipo ?p3 Hobbit)      ; ?p3 debe ser Hobbit
        (esDeTipo ?p4 Mago)        ; ?p4 debe ser Mago
        (esDeTipo ?p5 Elfo)        ; ?p5 debe ser Elfo
        (esMenor ?p1 ?p2)          ; ruptura de simetrías
        (esMenor ?p1 ?p3)          
        (esMenor ?p2 ?p3)
      )
    :effect
      (and
        ...
        (esMiembro ?p4)            ; ?p4 es miembro
        (esMiembro ?p5)            ; ?p5 es miembro
  ...
  ; ACCIÓN: viajarComunidad  (5 miembros)
  ;
  ; Mueve a los 5 miembros de la Comunidad conjuntamente de una localización a otra.
  ...
  ;   ?p4  - Cuarto miembro de la Comunidad
  ;   ?p5  - Quinto miembro de la Comunidad
  ;   ?org - Localización de origen
  ;   ?dst - Localización de destino
  ;
  ; Precondiciones:
  ...
  ;   - La comunidad está en la localización de origen
  ;   - Existe camino entre origen y destino.
  ;   - ?p1 < ?p2 < ?p3 para ruptura de simetrías.
  ;   - ?p1, ?p2 y ?p3 son Hobbits, ?p4 es un Mago y ?p5 es un Elfo. (para reducir el espacio de búsqueda)
  ;
  ; Efectos:
  ...
  (:action viajarComunidad
    :parameters (?p1 - Personaje ?p2 - Personaje ?p3 - Personaje ?p4 - Personaje ?p5 - Personaje ?org - Localizacion ?dst - Localizacion)
    :precondition
      (and
        ...
        (esMiembro ?p4)            ; ?p4 pertenece a la comunidad
        (esMiembro ?p5)            ; ?p5 pertenece a la comunidad
        (disponible ?p1)           ; ?p1 no está trabajando
        (disponible ?p2)           ; ?p2 no está trabajando
        (disponible ?p3)           ; ?p3 no está trabajando
        (disponible ?p4)           ; ?p4 no está trabajando
        (disponible ?p5)           ; ?p5 no está trabajando
        (comunidadEstaEn ?org)     ; la Comunidad está en el origen
        (camino ?org ?dst)         ; existe camino entre origen y destino
        (esMenor ?p1 ?p2)          ; ruptura de simetrías (optimización)
        (esMenor ?p1 ?p3)          
        (esMenor ?p2 ?p3)
