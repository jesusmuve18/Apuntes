...
func _ready() -> void:
	...
	# Asigamos coordenadas de textura a cada vértice de forma que sean funciones
	# lineales de las coordenadas (𝑥, 𝑦) de cada vértice.

	for i in range(n*(m+1)):
		uvs.append(Vector2((1+vertices[i].x)/2, (1-vertices[i].y)/2))
