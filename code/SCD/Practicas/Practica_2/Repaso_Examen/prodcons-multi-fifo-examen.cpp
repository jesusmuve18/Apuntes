...
void  funcion_hebra_productora( int num_hebra  )
{
   for( unsigned i = 0 ; i < num_items/num_prod ; i++ )
   {
      ...
      acceso_producidos.lock();
      n_producidos++;
      acceso_producidos.unlock();

      if(i%n_notificar==0 && i>0){
         espera_impresora.sem_signal();
         espera_productor_impresora.sem_wait();
      }
...
void funcion_hebra_impresora() 
{
   while(n_producidos<num_items){
      espera_impresora.sem_wait();

      cout_mtx.lock();
      cout<<"Se han producido "<<n_producidos<<" datos "<<endl;
      cout_mtx.unlock();

      espera_productor_impresora.sem_signal();
   }
}
...
int main()
{
   ...
   thread
      impresora=thread(funcion_hebra_impresora); // hebra impresora
